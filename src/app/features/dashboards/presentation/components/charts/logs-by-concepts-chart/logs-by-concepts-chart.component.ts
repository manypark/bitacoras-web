...
import { CustomSelectConceptsComponent } from "@app/shared";
...
@Component({
  ...
  imports     : [BaseChartDirective, CustomSelectConceptsComponent],
})
export class LogsByConceptsChartComponent {
  ...
  idConcepts = computed( () => this.selectedConcepts().join(',') );
  ...
  readonly query = injectQuery(() => ({
      queryKey: [ 'logsByConcepts', this.idConcepts() ],
      queryFn : () => this.logsByConceptUsecase.execute( this.idConcepts() ),
  }));

  // #=============== funciones ===============#
  clearSelect = () => this.selectedConcepts.set([]);
